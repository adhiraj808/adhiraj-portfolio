...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && config[mobileKey] !== undefined) v = config[mobileKey] as string | number;
      else if (w <= 768 && config[tabletKey] !== undefined) v = config[tabletKey] as string | number;
      else if (w <= 1024 && config[desktopKey] !== undefined) v = config[desktopKey] as string | number;
      setVal(v);
    };
...
export const GradualBlur: React.FC<PropsWithChildren<GradualBlurProps>> = (
  props
) => {
